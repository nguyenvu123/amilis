...
// add_action('updraftplus_settings_afternavtabs', array($this, 'settings_afternavtabs'));
	...
	public function options_printpage() {
		...
		if ('updraftplus' != basename(dirname(dirname(__FILE__)))) {
			echo '<div style="clear: both; float: left; font-size: 120%; padding: 8px; border: 1px solid;"><strong>'.__('Error', 'updraftplus').':</strong> '.sprintf(__("You have installed this plugin in your plugins folder (%s) with a non-default name %s which is different to %s. This is incompatible with WordPress's updates mechanism; you will not be able to receive updates.", 'updraftplus'), WP_PLUGIN_DIR, '<strong>'.basename(dirname(dirname(__FILE__))).'</strong>', '<strong>updraftplus</strong>').'</div>';
		}
